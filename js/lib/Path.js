...
(function() {
    ...
    Path.wxJump = function(url) {
        ...
        coAjax.post(appConfig.other.make_wx_short_url, {
            ...
            var domain = Path.getCookieRootDomain();
    ...
    //使用Cookie获取当前域名的Domain
    /*
    著作权归作者所有。
    商业转载请联系作者获得授权，非商业转载请注明出处。
    作者：匿名用户
    链接：https://www.zhihu.com/question/20994750/answer/25164668
    来源：知乎
    */
    var rootDomain;
    Path.getCookieRootDomain = function() {
        if (rootDomain) {
            return rootDomain;
        }
        ...
        if (rdl == 1) {
            ...
            while (rdl-- > 2) {
                ...
                tmpCookie = Cookies.get(tmpCookie);
